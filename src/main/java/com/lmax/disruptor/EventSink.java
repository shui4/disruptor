...
public interface EventSink<E> {
  /**
   * Publishes an event to the ring buffer. It handles claiming the next sequence, getting the
   * current (uninitialised) event from the ring buffer and publishing the claimed sequence after
   * translation.
   *
  ...
  /**
   * Publishes multiple events to the ring buffer. It handles claiming the next sequence, getting
   * the current (uninitialised) event from the ring buffer and publishing the claimed sequence
   * after translation.
   *
   * <p>With this call the data that is to be inserted into the ring buffer will be a field (either
   * explicitly or captured anonymously), therefore this call will require an instance of the
   * translator for each value that is to be inserted into the ring buffer.
   *
  ...
  /**
   * Attempts to publish an event to the ring buffer. It handles claiming the next sequence, getting
   * the current (uninitialised) event from the ring buffer and publishing the claimed sequence
   ...
   * getting the current (uninitialised) event from the ring buffer and publishing the claimed
   * sequence after translation. Will return false if specified capacity was not available.
   *
