...
/**
 * A {@link Sequence} group that can dynamically have {@link Sequence}s added and removed while
 * being thread safe.
 *
 * <p>The {@link SequenceGroup#get()} and {@link SequenceGroup#set(long)} methods are lock free and
 * can be concurrently be called with the {@link SequenceGroup#add(Sequence)} and {@link
...
public final class SequenceGroup extends Sequence {
  ...
  /**
   * Add a {@link Sequence} into this aggregate. This should only be used during initialisation. Use
   ...
   * It will set the sequences to cursor value of the ringBuffer just after adding them. This should
   * prevent any nasty rewind/wrapping effects.
   *
   * @param cursored The data structure that the owner of this sequence group will be pulling it's
   *     events from.
