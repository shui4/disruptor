...
/**
 * WorkerPool contains a pool of {@link WorkProcessor}s that will consume sequences so jobs can be
 * farmed out across a pool of workers. Each of the {@link WorkProcessor}s manage and calls a {@link
 * WorkHandler} to process the events.
 *
...
public final class WorkerPool<T> {
   ...
   *
   * <p>This option requires a pre-configured {@link RingBuffer} which must have {@link
   * RingBuffer#addGatingSequences(Sequence...)} called before the work pool is started.
   *
   * @param ringBuffer of events to be consumed.
   * @param sequenceBarrier on which the workers will depend.
   * @param exceptionHandler to callback when an error occurs which is not handled by the {@link
   *     WorkHandler}s.
  ...
  public WorkerPool(
    ...
    for (int i = 0; i < numWorkers; i++) {
      workProcessors[i] =
          new WorkProcessor<>(
              ringBuffer, sequenceBarrier, workHandlers[i], exceptionHandler, workSequence);
    }
  }
   ...
   *
   * <p>This option does not require {@link RingBuffer#addGatingSequences(Sequence...)} to be called
   * before the work pool is started.
   *
  ...
  public WorkerPool(
    ...
    for (int i = 0; i < numWorkers; i++) {
      workProcessors[i] =
          new WorkProcessor<>(ringBuffer, barrier, workHandlers[i], exceptionHandler, workSequence);
    }
