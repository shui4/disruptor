...
public class Disruptor<T> {
   ...
   *     #handleEventsWith(com.lmax.disruptor.EventHandler[])}, that will form the barrier for
   *     subsequent handlers or processors.
   ...
   *
   * <p>This call is additive, but generally should only be called once when setting up the
   * Disruptor instance
   *
  ...
  EventHandlerGroup<T> createWorkerPool(
    ...
    final WorkerPool<T> workerPool =
        new WorkerPool<>(ringBuffer, sequenceBarrier, exceptionHandler, workHandlers);
   ...
   *     #handleEventsWith(com.lmax.disruptor.EventProcessor...)}, that will form the barrier for
   *     subsequent handlers or processors.
  ...
  /**
   * Calls {@link com.lmax.disruptor.EventProcessor#halt()} on all of the event processors created
   * via this disruptor.
   */
  public void halt() {
