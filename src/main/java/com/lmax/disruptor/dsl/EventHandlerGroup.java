...
public class EventHandlerGroup<T> {
  ...
  /**
   * Set up batch handlers to consume events from the ring buffer. These handlers will only process
   * events after every {@link EventProcessor} in this group has processed the event.
   *
   * <p>This method is generally used as part of a chain. For example if the handler <code>A</code>
   * must process events before handler <code>B</code>:
   *
   * <pre><code>dw.handleEventsWith(A).then(B);</code></pre>
   *
   * @param handlers the batch handlers that will process events.
   * @return a {@link EventHandlerGroup} that can be used to set up a event processor barrier over
   *     the created event processors.
   */
  @SafeVarargs
  ...
  /**
   * Set up batch handlers to handle events from the ring buffer. These handlers will only process
   * events after every {@link EventProcessor} in this group has processed the event.
   *
   * <p>This method is generally used as part of a chain. For example if <code>A</code> must process
   * events before <code>B</code>:
   *
   ...
   */
  @SafeVarargs
  public final EventHandlerGroup<T> then(
  ...
  /**
   * Set up a worker pool to handle events from the ring buffer. The worker pool will only process
   * events after every {@link EventProcessor} in this group has processed the event. Each event
   * will be processed by one of the work handler instances.
   *
   * <p>This method is generally used as part of a chain. For example if the handler <code>A</code>
   * must process events before the worker pool with handlers <code>B, C</code>:
   *
   * <pre><code>dw.handleEventsWith(A).thenHandleEventsWithWorkerPool(B, C);</code></pre>
   *
   * @param handlers the work handlers that will process events. Each work handler instance will
   *     provide an extra thread in the worker pool.
   * @return a {@link EventHandlerGroup} that can be used to set up a event processor barrier over
   *     the created event processors.
   */
  @SafeVarargs
  public final EventHandlerGroup<T> thenHandleEventsWithWorkerPool(
      final WorkHandler<? super T>... handlers) {
   ...
   * <tt>otherHandlerGroup</tt>.
   *
   * @param otherHandlerGroup the event handler group to combine.
   * @return a new EventHandlerGroup combining the existing and new consumers into a single
   *     dependency group.
   */
  public EventHandlerGroup<T> and(final EventHandlerGroup<T> otherHandlerGroup) {
    final Sequence[] combinedSequences =
        new Sequence[this.sequences.length + otherHandlerGroup.sequences.length];
    System.arraycopy(this.sequences, 0, combinedSequences, 0, this.sequences.length);
    System.arraycopy(
        otherHandlerGroup.sequences,
        0,
        combinedSequences,
   ...
   * @return a new EventHandlerGroup combining the existing and new processors into a single
   *     dependency group.
   */
  ...
  /**
   * Create a dependency barrier for the processors in this group. This allows custom event
   * processors to have dependencies on {@link com.lmax.disruptor.BatchEventProcessor}s created by
   * the disruptor.
   *
