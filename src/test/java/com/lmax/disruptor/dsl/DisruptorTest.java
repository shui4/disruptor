...
public class DisruptorTest {
  ...
  private final Collection<DelayedEventHandler> delayedEventHandlers =
      new ArrayList<DelayedEventHandler>();
  ...
  public void should() throws Exception {
    ...
    BatchEventProcessor<TestEvent> b1 =
        new BatchEventProcessor<TestEvent>(rb, rb.newBarrier(), new SleepingEventHandler());
    EventProcessorFactory<TestEvent> b2 =
  ...
  public void shouldBatchOfEvents() throws Exception {
    ...
    if (!eventCounter.await(5, TimeUnit.SECONDS)) {
      fail(
          "Did not process event published before start was called. Missed events: "
  ...
  public void shouldHonourDependenciesForCustomProcessors() throws Exception {
    ...
    disruptor
        .handleEventsWith(delayedEventHandler)
        .then(
            new EventProcessorFactory<TestEvent>() {
              @Override
              public EventProcessor createEventProcessor(
    ...
    disruptor.handleEventsWith(
        new EventProcessorFactory<TestEvent>() {
          ...
          public EventProcessor createEventProcessor(
            ...
            return new BatchEventProcessor<TestEvent>(
                disruptor.getRingBuffer(), ringBuffer.newBarrier(barrierSequences), eventHandler);
          }
        });
  ...
  private void assertThatCountDownLatchEquals(
    ...
    assertThat(
        Long.valueOf(countDownLatch.getCount()), equalTo(Long.valueOf(expectedCountDownValue)));
  }
  ...
  @Test
  public void
      shouldSupportCombiningWorkerPoolWithEventHandlerAsDependencyWhenNotPreviouslyRegistered()
          throws Exception {
    final TestWorkHandler workHandler1 = createTestWorkHandler();
  ...
  public void shouldSupportUsingWorkerPoolWithADependency() throws Exception {
    ...
    disruptor
        .handleEventsWith(delayedEventHandler)
        .thenHandleEventsWithWorkerPool(workHandler1, workHandler2);
  ...
  public void shouldTrackRemainingCapacity() throws Exception {
    ...
    // Given
    final EventHandler<TestEvent> eventHandler =
        new EventHandler<TestEvent>() {
          @Override
          public void onEvent(final TestEvent event, final long sequence, final boolean endOfBatch)
              throws Exception {
