...
class BatchedPoller<T> {
  ...
  private EventPoller.PollState loadNextValues(
      EventPoller<BatchedPoller.DataEvent<T>> poller, final BatchedData<T> batch) throws Exception {
    return poller.poll(
        new EventPoller.Handler<BatchedPoller.DataEvent<T>>() {
          @Override
          public boolean onEvent(
              BatchedPoller.DataEvent<T> event, long sequence, boolean endOfBatch)
              throws Exception {
